...
def final_eval(df_train, df_test, model, target_column='DFT_Barrier', print_pred=False, name_out='preds'):
    """
    ...
    feature_names = [column for column in df_train.columns if column not in['rxn_id', 'DG_TS', 'G_r', 'DG_TS_tunn', 'DFT_Barrier', 'G_act']]
    ...
    if print_pred:
        np.savetxt(f'tmp/{name_out}.txt', target_scaler.inverse_transform(predictions))
...
def prepare_df(data, features):
    ...
    features = features + ['rxn_id'] + ['DG_TS'] + ['DFT_Barrier'] + ['G_act']
...
def cross_val(df, model, n_folds, target_column='DG_TS_tunn', sample=None, split_dir=None):
    ...
    feature_names = [column for column in df.columns if column not in['rxn_id', 'DG_TS', 'G_r', 'DG_TS_tunn', 'Barrier', 'file_name', 'gibbs_exp']]
