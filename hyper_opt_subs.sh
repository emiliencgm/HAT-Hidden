...
##SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=2 # TODO add more CPUs
    ...
    for hidden_size in "${hidden_sizes[@]}"; do
        for dropout in "${dropouts[@]}"; do
            for lr in "${lrs[@]}"; do
                ...
                # TODO select a dataset and add more CPUs for srun
                srun --ntasks=1 --cpus-per-task=2 python -u main-emilien.py --dataset in_house --chk_path_hidden 1200 --fine_tune 1 \
...
#                     srun --ntasks=1 --cpus-per-task=2 python -u main-emilien.py --dataset in_house --chk_path_hidden $hidden_size_M1 --fine_tune 1 \
#                         --layers "$layer" --hidden_size "$hidden_size" --dropout "$dropout" --lr "$lr" &
