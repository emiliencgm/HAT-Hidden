...
import argparse
from reactivity_model.data import PredDataset_hidden
...
def predict_hidden(test_file='tmp/species_reactivity_dataset.csv'):
...
def retrain_in_house(args):
    ...
    hyper_opt_cv_hidden(features=args.features, target_column='dG_act_corrected', save_dir='tmp/cv_in-house_data', k_fold=10, ensemble_size=4, data_path='tmp/input_ffnn_hidden.pkl', layers=args.layers, hidden_size=args.hidden_size, dropout=args.dropout, lr=args.lr, logger=logger, random_state=args.random_state)
...
def hyper_opt_cv_hidden(transfer_learning=False, trained_dir='tmp/cv_in-house_data/fold_0', features=['mol1_hidden','rad2_hidden','rad_atom2_hidden'], target_column='DFT_Barrier', save_dir='tmp/cv_tantillo_data', k_fold=4, ensemble_size=4, random_state=0, data_path = None, test_set=None, train_valid_set=None, layers=1, hidden_size=1024, dropout=0.0, lr=0.0277, gpu=True, logger=None, max_epochs=100):
    ...
    cv = {"RMSE" : rmse, "MAE": mae, "R2": r2}
    args = {"features":features, "layers":layers, "hidden_size":hidden_size, "dropout":dropout, "lr":lr, "transfer_learning":transfer_learning, "ensemble_size":ensemble_size, "max_epochs":max_epochs, "random_state":random_state}
    out_yaml = {"args": args, f"{k_fold}-fold cross-validation results": cv}
    out_str = yaml.dump(out_yaml, indent=2, default_flow_style=False)
    with open(Path(save_dir) / "cv_hyper_opt_simple.yaml", "a") as fp:
        fp.write(out_str)

def hyper_opt_up_hidden(save_dir='tmp/cv_tantillo_data', data_path = None, train_valid_set_path = 'tmp/tantillo_data/input_tantillo_hidden.pkl', test_set_path = 'tmp/tantillo_data/input_steroids_tantillo_hidden.pkl', trained_dir = 'tmp/cv_own_dataset/fold_0', transfer_learning = False, target_column='DFT_Barrier', ensemble_size=4, batch_size=64, layers = 0, hidden_size=256, dropout=0.0, lr=0.0277, random_state=0, lr_ratio=0.95, features=['mol1_hidden','rad2_hidden', 'rad_atom2_hidden'], max_epochs=100, gpu=True, logger=None):
    
    run_train_M2(save_dir=save_dir, data_path = data_path, train_valid_set_path = train_valid_set_path, test_set_path = test_set_path, trained_dir = trained_dir, transfer_learning = transfer_learning, target_column=target_column, ensemble_size=ensemble_size, batch_size=batch_size, layers = layers, hidden_size=hidden_size, dropout=dropout, lr=lr, random_state=random_state, lr_ratio=lr_ratio, features=features, max_epochs=max_epochs, gpu=gpu)
...
def retrain_tantillo_Cytochrome_P450_reprod(args):
    ...
    # NOTE　Transfer Learning loading dir: input_dim=3600 !!!
    #@@@ train : test = 18 : 6
    # hyper_opt_up_hidden(save_dir='tmp/cv_tantillo_data', data_path = None, train_valid_set_path = 'tmp/tantillo_data/input_tantillo_hidden.pkl', test_set_path = 'tmp/tantillo_data/input_steroids_tantillo_hidden.pkl', trained_dir = 'tmp/cv_own_dataset/fold_0', transfer_learning = False, target_column='DFT_Barrier', ensemble_size=4, batch_size=64, layers = args.layers, hidden_size=args.hidden_size, dropout=args.dropout, lr=args.lr, random_state=0, lr_ratio=0.95, features=['mol1_hidden','rad2_hidden', 'rad_atom2_hidden'], max_epochs=100, gpu=True, logger=logger)
    
    #@@@ train : val : test = 16 : 2 : 6
    hyper_opt_up_hidden(save_dir='tmp/cv_tantillo_data', data_path = 'tmp/tantillo_data/input_tantillo_hidden.pkl', train_valid_set_path = None, test_set_path = 'tmp/tantillo_data/input_steroids_tantillo_hidden.pkl', trained_dir = 'tmp/cv_own_dataset/fold_0', transfer_learning = False, target_column='DFT_Barrier', ensemble_size=4, batch_size=64, layers = args.layers, hidden_size=args.hidden_size, dropout=args.dropout, lr=args.lr, random_state=args.random_state, lr_ratio=0.95, features=args.features, max_epochs=100, gpu=True, logger=logger)
...
def retrain_Hong_Photoredox(args):
    ...
    hyper_opt_cv_hidden(data_path='tmp/input_ffnn_hidden.pkl', target_column='DG_TS', save_dir='tmp/cv_hong_data',k_fold=5, ensemble_size=4, layers=args.layers, hidden_size=args.hidden_size, dropout=args.dropout, lr=args.lr, features=args.features, logger=logger, random_state=args.random_state)

def retrain_exp_Omega_alkoxy(args, fold_i=0):
    logger = create_logger('omega_exp_data.log')
    logger.info('********************************')
    logger.info('======= OMEGA_EXP DATASET =======')
    predict_hidden(test_file='tmp/omega_data/species_reactivity_omega_dataset.csv')
...
def retrain_omega_bietti_hong(args):
    ...
    logger.info('********************************')
    logger.info('======= OMEGA_BIETTI_HONG DATASET =======')
    predict_hidden(test_file='tmp/omega_data/species_reactivity_omega_dataset.csv')
    ...
    hyper_opt_cv_hidden(transfer_learning=False, trained_dir='tmp/cv_hong_data/fold_0', features=args.features, target_column='gibbs_exp', save_dir='tmp/cv_bietti_data', k_fold=10, ensemble_size=4, data_path='tmp/input_ffnn_hidden.pkl', layers=args.layers, hidden_size=args.hidden_size, dropout=args.dropout, lr=args.lr, logger=logger, random_state=args.random_state)
...
def retrain_RMechDB(args):
    ...
    hyper_opt_cv_hidden(transfer_learning=True, trained_dir='tmp/cv_in-house_data/fold_0', data_path='tmp/input_ffnn_hidden.pkl', target_column='DG_TS_tunn', save_dir='tmp/cv_rmechdb_data',k_fold=10, ensemble_size=4, layers=args.layers, hidden_size=args.hidden_size, dropout=args.dropout, lr=args.lr, features=args.features, logger=logger, random_state=args.random_state)    
...
if __name__ == '__main__':    
    ...
    reproduce_hidden(dataset=args.dataset, args=args, fine_tune=False)
    # reproduce_RMechDB()
