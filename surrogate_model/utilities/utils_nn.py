...
class NoamLR(_LRScheduler): # is never instantiated
    """
...
class SinexpLR(_LRScheduler):
    """
    Sinexp learning rate scheduler with exponential decay and sine wave oscillation of the lr.

    The maximum of the sine wave decays exponentially over the epochs. (Not the original doc)
    """
    def __init__(self,
                 optimizer: Optimizer,
                 total_epochs: List[int],
        ...
        :param final_lr: The final learning rate (achieved after total_epochs).
        """
        assert len(optimizer.param_groups)  == len(total_epochs) == len(init_lr) == len(final_lr)
        ...
        self.period_l = np.array([500]) # describes oscillation nature
...
class LRRange(_LRScheduler):
    """
    Learning rate scheduler with linear increase of the lr. This is the LR Range Test to obtain 
    optimal boundary lr for the cyclic LRS as recommended by Leslie Smith (https://arxiv.org/abs/1506.01186).
    """
    def __init__(self,
                 optimizer: Optimizer,
                 total_epochs: List[int],
        ...
        self.lr = init_lr
        self.total_steps = self.total_epochs * self.steps_per_epoch
        self.slope = (self.final_lr-self.init_lr)/self.total_steps

        super(LRRange, self).__init__(optimizer)
    ...
    def step(self, current_step: int = None):
        ...
        for i in range(self.num_lrs):
            if self.current_step <= self.total_steps[i]:
                self.lr[i] = self.slope[i] * self.current_step + self.init_lr[i]
            else:  # theoretically this case should never be reached since training should stop at total_steps
                self.lr[i] = self.final_lr[i]
